...
## Goals
# Create a new WSL2 instance from a root filesystem tarball.
# allowing configuration from Ansible playbooks.
# regular user shares SSH config from Windows host.
# regular user has KeeAgent support from KeePass.
...
function Set-SshConfigSharing {
    ...
    Write-Information "Creating symbolic link for SSH configuration in WSL2 instance '$InstanceName'."
    Invoke-BashCommandInWsl2 `
        -InstanceName $InstanceName `
        -User $env:USERNAME `
        -Command "ln -s `$USERPROFILE/.ssh ~/.ssh"

    Write-Information "Setting permissions for SSH configuration in WSL2 instance '$InstanceName'."
    Invoke-BashCommandInWsl2 `
        -InstanceName $InstanceName `
        -User $env:USERNAME `
        -Command "chmod 700 ~/.ssh && cd ~/.ssh && chmod 0700 .. && chmod 0600 ~/.ssh/*"
}
...
function Set-KeeAgentSupport {
    ...
    Set-WslSshAgentForwrder -InstanceName $InstanceName
}
...
function Set-WslSshAgentForwrder {
    param (
        [string] $InstanceName
    )
